...
function removeImage(cards){
    let newCards = []
   for (let i =0; i < cards.length; i++  ) {
       delete cards[i].images
  newCards.push(cards[i])
   }
  return newCards
}
console.log(removeImage(cards))



//array.prototype
// start as 0 - Don't forget!!
// I will always be a number

//2.

function removeSpades(array) {
    const cards=[]
...
console.table(removeSpades(cards))


*/

//Friday
//Parameter ways to pass data through the function.
//Looping task
// i == cards

function sortCardBySuit(array) {
    const sortedCards = {clubs: [], spades: [], diamonds: [], hearts: []}
    for (let i = 0; i < array.length; i++) {
        if (array[i].suit === "HEARTS") {
            sortedCards.hearts.push(array[i])
        } else if(array[i].suit === "CLUBS") {
            sortedCards.clubs.push(array[i])
        } else if(array[i].suit === "SPADES") {
            sortedCards.spades.push(array[i])
        } else if(array[i].suit === "DIAMONDS") {
            sortedCards.diamonds.push(array[i])
        }
    }
    return sortedCards
}

const result = sortCardBySuit(cards)
